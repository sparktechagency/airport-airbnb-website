    ...
    return (
        <div className="lg:p-8 p-5">
            ...
            <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-[40px] pb-[60px]">
                ...
                {
                    data?.map((item) => (
                        <div key={item.id} className={` bg-white rounded-lg py-5 shadow-md px-2 ${activeType === item?.id && "border border-primary "} `} onClick={() => { setActiveType(item?.id); setDesignation(item?.type) }} >
            ...
            <div className="flex flex-col items-center justify-center gap-7">
                <button className=" bg-primary text-white text-[16px] font-semibold rounded-full w-full h-12" onClick={handleVerifiedAccess} > Verified Access </button>

                <button className=" bg-white text-gray-500 border border-[#E0E0E0] text-[16px] font-medium rounded-full w-full h-12" onClick={() => router.push("/subscription-plan")}> Skip </button>
            </div>
